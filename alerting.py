...
class AlertManager:
    ...
    def __init__(self, environ=None):
        ...
        self._cooldown_seconds = float(self.environ.get("ALERT_COOLDOWN_SECONDS", "60"))
    ...
    def _dispatch(self, subject, body, channel_keys=None):
        ...
        for channel in channels:
            ...
            cooldown_key = f"{channel['key']}:{subject_prefix}"
            with self._lock:
                last_ts = self._channel_state.get(cooldown_key, {}).get("last_dispatch_ts", 0)
            elapsed = now - last_ts
            if elapsed < cooldown:
                remaining = cooldown - elapsed
                print(f"[Alert] Suppressed {channel['label']} for '{subject_prefix}' "
                ...
                results.append({
                    ...
                    "transport": channel["transport"],
                    "ok": True,
                    "message": f"Suppressed by cooldown ({remaining:.0f}s remaining)",
                })
                continue
            ...
            try:
                ...
                with self._lock:
                    self._channel_state.setdefault(cooldown_key, {})["last_dispatch_ts"] = now
